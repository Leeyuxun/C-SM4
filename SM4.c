...
void encode_fun(u8 len,u8 *key, u8 *input, u8 *output)
{
	...
	for (j = 0; j < len / 16 + ((len % 16) ? 1:0); j++)  //进行循环加密,并将加密后数据保存（以16字节为一次加密，进行循环，即若16字节则进行一次，17字节补0至32字节后进行加密两次，以此类推）
	{
		/*开始处理加密数据*/
...
int main(void)
{
	...
	u8 key[16] = { 0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef }; 	//定义16字节的密钥
	printf("%s\n",key); 
	u8 Data_plain[] = "c_sm4"; 	//待加密数据 
	printf("加密前数据：%s\n",Data_plain);
	...
	for (i = 0; i < len ; i++)
	{
		if(encode_Result[i] >= 16)
		{
			printf("0x%x ", encode_Result[i]);
		}
		else{
			printf("0x0%x ", encode_Result[i]);
		}
	}
	printf("\n");
